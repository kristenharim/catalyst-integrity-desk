...
| `phase3-receipt-from-ledger.json` | Phase 3, the receipt | Closing a forgery. The receipt was passed through the redirect as JSON in the query string, so a hand-written URL rendered author `nobody:forged` and an entry hash of `deadbeef` repeated, presented as cryptographic evidence beside a badge reading intact. Now built from the ledger's last entry at render time, the same way the badge is. Same defect class as the badge had, found the same way. |
| `phase3-ledger-anchor.json` | Phase 3, the anchor | Making deletion detectable. A three-model adversarial review predicted that `verify()` proves only that the presented chain is self-consistent, and the prediction held: deleting the newest entry, and replacing the whole file with a valid forged chain, both returned `True`. `orchestrator/anchor.py` records the head hash and entry count outside the chain. The badge now has three states, and a missing anchor fails closed. |
| `phase3-real-breach.json` | Phase 3, the real breach | Deleting the scripted amendment. The engine fix had made it obsolete: Rocket was already at -14.5 before any amendment, so a scripted +9 month shift showed an already-broken thesis getting worse. Replaced with the event that actually happened, the approved date lapsing, which needed no invention at all. Also the middot typography pass. |
